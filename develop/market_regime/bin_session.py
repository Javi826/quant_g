"""
Session bin search — classify signals by trading session (UTC) and find
consistent session combos to filter across train periods, validate on OOS1.

Sessions (UTC):
  asian_early    00:00 - 04:00
  asian_late     04:00 - 08:00
  european_early 08:00 - 12:00
  european_late  12:00 - 16:00
  american_early 16:00 - 20:00
  american_late  20:00 - 00:00
...
BINS_OUTPUT_PATH     = os.path.join(os.path.dirname(__file__), f"regime_bins_session_{STRATEGIES_SET_NAME}.py")
...
MATCH_MODE        = 'intersection'   # 'exact' | 'intersection'
MIN_PERIODS_MATCH = 2
...
SESSIONS = {
    ...
    'european_early': (8,  12),
    'european_late':  (12, 16),
    'american_early': (16, 20),
    'american_late':  (20, 24),
}

ALL_BINS = list(SESSIONS.keys())
...
_BIN_ABBREV = {
    'asian_early':    'asi_e',
    'asian_late':     'asi_l',
    'european_early': 'eur_e',
    'european_late':  'eur_l',
    'american_early': 'ame_e',
    'american_late':  'ame_l',
}
...
def _get_session(ts: pd.Timestamp) -> str:
    ...
    for session, (start, end) in SESSIONS.items():
        if start <= hour < end:
            return session
    return 'asian_early'  # fallback for hour==0 edge case
...
# =============================================================================
# SIGNAL CLASSIFICATION BY SESSION
# =============================================================================
...
def _classify_signals(
    ...
    """
    Classify each signal by trading session based on its timestamp.
    Returns {sym: {'signals': np.ndarray, 'signal_bins': dict{idx: session}, 'arr': arr}}
    """
    result = {}
    ...
    # Filter out sessions with fewer than MIN_SIGNALS_PER_BIN signals across all symbols
    bin_counts: dict[str, int] = {}
    for sym_data in result.values():
        for session in sym_data['signal_bins'].values():
            bin_counts[session] = bin_counts.get(session, 0) + 1
...
def _run_backtest_with_bins(
    ...
    for sym, data in classified.items():
        ...
        for idx, session in data['signal_bins'].items():
...
def run_session_search(train_keys: list[str], val_key: str) -> None:
    _t0 = time.time()

    print(f"\n{'='*100}")
    print(f"  SESSION BIN SEARCH — Train: {' + '.join(train_keys)}  →  Validate: {val_key}")
    print(f"  Sessions (UTC): {', '.join(f'{s}={v[0]:02d}-{v[1]:02d}h' for s, v in SESSIONS.items())}")
...
def _save_bins(bins_per_strategy: dict[str, set], train_keys: list[str]) -> None:
    lines = [
        f"# Auto-generated by bin_session.py",
        f"# Sessions (UTC): {', '.join(f'{s}={v[0]:02d}-{v[1]:02d}h' for s, v in SESSIONS.items())}",
        f"# Train: {' + '.join(train_keys)}",
        f"# Match mode: {MATCH_MODE}" + (f" | min_periods={MIN_PERIODS_MATCH}" if MATCH_MODE == 'exact' else ""),
        f"",
        f"SESSIONS = {dict(SESSIONS)}",
        f"",
        f"REGIME_BINS = {{",
    ]
...
if __name__ == "__main__":
    run_session_search(TRAIN_KEYS, VALIDATE_KEY)