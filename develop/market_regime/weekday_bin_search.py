# bitget/develop/market_regime/weekday03_bin_search.py
"""
Weekday optimization — exhaustive bin combination search.

For each strategy/period:
  1. Pre-classify every signal by weekday bin (once) based on UTC day of week
  2. Run baseline backtest (once)
  3. Evaluate all 128 bin combinations (2^7) — each requires one backtest
  4. Pick the combination that maximizes profit on train periods
  5. Validate on held-out period

Weekday bins are determined purely by signal timestamp (UTC day of week) —
...
from shared_batchs.regime.weekday_module import ALL_WEEKDAY_BINS, classify_signal_by_weekday
...
BINS_OUTPUT_PATH     = os.path.join(os.path.dirname(__file__), f"weekday_bins_03_{STRATEGIES_SET_NAME}.py")
...
# =============================================================================
# MODE
# 'search_by_tf'  — find best bin combo to filter (exhaustive 128 combos)
# 'analyze_by_tf' — use FIXED_BINS_BY_STRATEGY, persist
# =============================================================================
MODE = 'filter_single_day'

# Fixed bins per strategy for MODE='analyze_by_tf'
FIXED_BINS_BY_STRATEGY: dict[str, set] = {}
SINGLE_DAY_FILTER: str = 'thursday'  # used when MODE='filter_single_day'

# Search params
ORDER_AMOUNT        = 80
MIN_SIGNALS_PER_BIN = 50

# All 128 combinations (empty set + 2^7 - 1 non-empty)
ALL_WEEKDAY_BIN_COMBOS = [set()] + [
    set(combo)
    for r in range(1, len(ALL_WEEKDAY_BINS) + 1)
    for combo in combinations(ALL_WEEKDAY_BINS, r)
]
...
def _abbrev_bins(bins: set) -> str:
    if not bins:
        return '—'
    order = {b: i for i, b in enumerate(ALL_WEEKDAY_BINS)}
    return ', '.join(_BIN_ABBREV.get(b, b) for b in sorted(bins, key=lambda b: order[b]))
...
# =============================================================================
# SIGNAL CLASSIFICATION — pre-classify all signals by weekday bin (once)
# =============================================================================

def _classify_signals_by_weekday(
    ohlcv_arrays:  dict,
    signal_fn,
    signal_params: dict,
) -> dict[str, dict]:
    """
    For each symbol, generate signals and classify each signal into a weekday bin
    based on its UTC day of week. No OHLCV reference data required.
    Returns {sym: {'signals': np.ndarray, 'signal_bins': dict{idx: bin_key}, 'arr': arr}}
    Pre-computed once — reused across all 128 bin combinations.
    """
    result = {}
    for sym, arr in ohlcv_arrays.items():
        ...
        for idx in signal_idxs:
            ...
            weekday               = classify_signal_by_weekday(t)
            signal_bins[int(idx)] = weekday
...
def _run_backtest_with_bins(
    ...
    """
    Apply weekday bin filter to pre-classified signals and run backtest.
...
def _load_period_data(strategy: dict, period_key: str) -> dict | None:
    ...
    classified = _classify_signals_by_weekday(
...
# =============================================================================
# BIN SEARCH — evaluate all 128 combinations on train periods
# =============================================================================
...
def _find_best_bins(
    ...
    """
    Evaluate all 128 weekday bin combinations across train periods.
    ...
    for bins_combo in ALL_WEEKDAY_BIN_COMBOS:
        total_trades = 0
        profit_total = 0.0
...
def run_search_by_tf(train_keys: list[str], val_key: str) -> None:
    ...
    print(f"\n{'='*100}")
    print(f"  WEEKDAY BIN SEARCH — Train: {' + '.join(train_keys)}  →  Validate: {val_key}")
    print(f"  Bin combinations: {len(ALL_WEEKDAY_BIN_COMBOS)}")
    print(f"{'='*100}\n")
    ...
    for tf, strategies in sorted(by_tf.items()):
        print(f"\n{'─'*100}")
        print(f"  TF: {tf} | {len(strategies)} strategies | {len(ALL_WEEKDAY_BIN_COMBOS)} combos × {len(train_keys)} periods")
        print(f"{'─'*100}")
    ...
    _save_weekday_bins(bins_per_strategy, train_keys)
...
def run_analyze_by_tf(train_keys: list[str], val_key: str, fixed_bins: dict[str, set]) -> None:
    ...
    print(f"\n{'='*100}")
    print(f"  WEEKDAY ANALYZE — Train: {' + '.join(train_keys)}  →  Validate: {val_key}")
    print(f"{'='*100}\n")

    strategies_all = load_strategies_config()
    _run_validation_period(strategies_all, val_key, fixed_bins)
    _save_weekday_bins(fixed_bins, train_keys)
...
def _run_validation_period(
    ...
    print(f"\n{'='*100}")
    print(f"  VALIDATION RESULTS — {val_key}")
    print(f"{'='*100}")
    ...
    for strategy in strategies:
        sid  = strategy['id']
        data = _load_period_data(strategy, val_key)
        if not data:
            continue
...
def _save_weekday_bins(bins_per_strategy: dict[str, set], train_keys: list[str]) -> None:
    lines = [
        f"# Auto-generated by weekday03_bin_search.py",
    ...
    with open(BINS_OUTPUT_PATH, "w") as f:
        f.write("\n".join(lines) + "\n")
    print(f"  ✅ Weekday bins saved to: {BINS_OUTPUT_PATH}")
...
if __name__ == "__main__":
    ...
    elif MODE == 'filter_single_day':
        fixed = {s["id"]: {SINGLE_DAY_FILTER} for s in load_strategies_config()}
        run_analyze_by_tf(TRAIN_KEYS, VALIDATE_KEY, fixed)