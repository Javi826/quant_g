...
try:
    while True:
        print(f'\n🔷 === 04_{STRATEGY}_{TIMEFRAME_MINOR} strategy === 🔷')
        wait_for_next_candle(TIMEFRAME_MINOR)
    
        # 🔍 SINCRONIZAR con el exchange (detecta cierres por TP/SL)
        sync_positions_with_exchange(open_positions, get_open_positions_04, PRODUCT_TYPE)
        ...
        # ORDERS MANAGEMENT
        # -------------------------------
        manage_open_positions(open_positions, send_request_fn=send_request_04, product_type=PRODUCT_TYPE)
        save_state(open_positions, STATE_FILE)
