...
def run_grid_backtest(
    ...
    positions_value = 0.0  # running total de posiciones abiertas
    
    for t_int in all_timestamps_int:
        price_cache = {}  # cache de precios por tick
        ...
        while open_heap and open_heap[0][0] <= t_int:
            ...
            if 'exec_price' in pos and ('exec_time_int' in pos) and pos['exec_time_int'] <= t_int:
                ...
                exec_time_dt = pos['exec_time']
                exit_reason = pos['exit_reason']
            else:
                sell_ts_int = pos.get('sell_time_int', int(sym_data_local[sym]['ts_int'][-1]))
                key = (sym, sell_ts_int)
                if key in price_cache:
                    exec_price = price_cache[key]
                else:
                    exec_price = get_price_at_int(sym, sell_ts_int, sym_data_local, ts_index_map_by_sym_int)
                    price_cache[key] = exec_price
                exec_time_dt = np.datetime64(int(sell_ts_int), 'ns')
                exit_reason = 'SELL_AFTER' if exec_price is not None else 'FORCED_LAST'
                if exec_price is None:
                    exec_price = float(sym_data_local[sym]['close'][-1])
                    exec_time_dt = sym_data_local[sym]['ts'][-1]
    
            # Actualizar running total
            positions_value -= pos['qty'] * exec_price
    
            close_position(pos, exec_time_dt, exec_price, exit_reason)
            pos['closed'] = True
