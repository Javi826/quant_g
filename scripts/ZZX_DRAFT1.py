...
# ====================================================
# Integración directa con módulo Rust
# ====================================================
import numpy as np
import rust_backtest_helpers as _rbh

def _ensure_np_arrays(ts_arr, close_arr):
    """Asegura dtype y contigüidad para arrays numpy -> Rust"""
    ts = np.asarray(ts_arr, dtype=np.int64)
...
def get_price_at_int(sym, t, sym_data, ts_int_arrays, close_arrays):
    """
    Versión 100 % Rust: usa directamente get_price_at_int_fast del módulo compilado.
    """
    t_int = np.int64(t)
    ts_arr = ts_int_arrays[sym]
    close_arr = close_arrays[sym]
    ts_arr, close_arr = _ensure_np_arrays(ts_arr, close_arr)

    # Llamada directa a la función Rust
    return _rbh.get_price_at_int_fast(ts_arr, close_arr, int(t_int))
...
def compute_post_backtest_metrics(symbols, trades, trade_times, all_timestamps_dt, initial_balance, sim_balance_cols):
    all_timestamps_int64 = all_timestamps_dt.astype('int64')
    ...
    sim_ts_int64 = sim_ts_arr.astype('int64')
    ts_index_map = {t: i for i, t in enumerate(all_timestamps_dt)}
    ...
    max_dd_portfolio = float(np.max(drawdowns_portfolio)) if sim_values.size > 0 else 0.0

    # Calcular Sharpe usando la función externa
    sharpe_by_symbol = {sym: compute_annualized_sharpe(equity_by_symbol.get(sym), all_timestamps_int64) for sym in symbols}
    sharpe_portfolio = compute_annualized_sharpe(sim_values, sim_ts_int64)
    ...
    return {
        "final_balance": final_balance,
        "final_balance_by_symbol": final_balance_by_symbol,
        "max_dd_by_symbol": max_dd_by_symbol,
        "sharpe_by_symbol": sharpe_by_symbol,
