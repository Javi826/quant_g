...
def explosive_signal_39(high_mayor, close_mayor, high_menor, close_menor, lookback_mayor, lookback_menor, live=False):
    # Señales individuales
    signal_long_mayor = np.zeros_like(close_mayor, dtype=np.int8)
    signal_short_mayor = np.zeros_like(close_mayor, dtype=np.int8)
    signal_long_menor = np.zeros_like(close_menor, dtype=np.int8)
    signal_short_menor = np.zeros_like(close_menor, dtype=np.int8)
    ...
    for i in range(lookback_mayor, n_mayor):
        ...
        # Señal long
        if np.all(window[:-1] > window[1:]):
            if close_mayor[i] > np.max(window):
                signal_long_mayor[i] = 1
        # Señal short
        if np.all(window[:-1] < window[1:]):
            if close_mayor[i] < np.min(window):
                signal_short_mayor[i] = -1
    ...
    for i in range(lookback_menor, n_menor):
        ...
        # Señal long
        if np.all(window[:-1] > window[1:]):
            if close_menor[i] > np.max(window):
                signal_long_menor[i] = 1
        # Señal short
        if np.all(window[:-1] < window[1:]):
            if close_menor[i] < np.min(window):
                signal_short_menor[i] = -1

    # Combinación multi-timeframe
    factor = len(close_menor) // len(close_mayor)
    signal_final = np.zeros_like(close_menor, dtype=np.int8)

    for i in range(len(close_menor)):
        idx_mayor = i // factor
        # Señal long
        if idx_mayor < len(signal_long_mayor) and signal_long_menor[i] == 1 and signal_long_mayor[idx_mayor] == 1:
            signal_final[i] = 1
        # Señal short
        if idx_mayor < len(signal_short_mayor) and signal_short_menor[i] == -1 and signal_short_mayor[idx_mayor] == -1:
            signal_final[i] = -1
