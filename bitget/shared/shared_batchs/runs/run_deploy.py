...
from shared_batchs.tools.wfo_ST import WARMUP_BARS, _select_window_symbols
...
def _slice_deploy_window(
    ...
    return ohlcv_window, train_start_ts
...
def run_deploy_train(
    ...
    """
    Run the deploy train window for a strategy: select symbols, fit params,
    persist symbols_live and the PR batch file.
    Returns True if the deploy symbol list changed compared to the previous run.
    """

    logger.info(f"DEPLOY ── {strategy_id} ── running deploy train window")

    ohlcv_window, train_start_ts = _slice_deploy_window(ohlcv_is, timeframe)
    ohlcv_selected = _select_deploy_symbols(ohlcv_window, n_symbols, train_start_ts)
    deploy_symbols = list(ohlcv_selected.keys())
    logger.info(f"DEPLOY ── {strategy_id} ── {len(deploy_symbols)} symbols | from {pd.Timestamp(train_start_ts)}")
    ...
    deploy_map[strategy_id] = {"params": deploy_params, "approved": approved_wfo}
