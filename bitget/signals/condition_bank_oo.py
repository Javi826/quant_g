...
# =============================================================================
#     DEFAULT_RSI_PERIODS    = [14,21]
#     DEFAULT_ADX_PERIODS    = [14]
#     DEFAULT_RSI_THRESHOLDS = [30, 50, 70]
#     DEFAULT_ADX_THRESHOLDS = [20, 25]
#     DEFAULT_MA_PERIODS     = [20, 50, 100]
#     DEFAULT_MOMENTUM_N     = [5]
# =============================================================================
    
    DEFAULT_RSI_PERIODS    = [14]
    DEFAULT_ADX_PERIODS    = [14]
    DEFAULT_RSI_THRESHOLDS = [50]
    DEFAULT_ADX_THRESHOLDS = [25]
    DEFAULT_MA_PERIODS     = [100]
    DEFAULT_MOMENTUM_N     = [5]

    def __init__(
        self,
        arr: dict,
        rsi_periods: list    = DEFAULT_RSI_PERIODS,
        adx_periods: list    = DEFAULT_ADX_PERIODS,
        ...
        self._rsi_cache = {period: _rsi(self.close, period) for period in self.rsi_periods}
        self._adx_cache = {period: _adx(self.high, self.low, self.close, period) for period in self.adx_periods}
    ...
    def build_condition_specs(self) -> list:
        ...
        for period in self.rsi_periods:
            for th in self.rsi_thresholds:
                specs.append({"type": "rsi", "period": period, "op": ">", "value": th})
                specs.append({"type": "rsi", "period": period, "op": "<", "value": th})

        for period in self.adx_periods:
            for th in self.adx_thresholds:
                specs.append({"type": "adx", "period": period, "op": ">", "value": th})
    ...
    def evaluate(self, spec: dict) -> np.ndarray:
        ...
        if ctype == "rsi":
            series = self._rsi_cache[spec["period"]]
            return series > value if op == ">" else series < value

        if ctype == "adx":
            series = self._adx_cache[spec["period"]]
            return series > value
    ...
    def describe(self, spec: dict) -> str:
        ...
        if ctype == "rsi":
            return f"RSI{spec['period']}{op}{value}"
        if ctype == "adx":
            return f"ADX{spec['period']}{op}{value}"
        if ctype == "ma":
