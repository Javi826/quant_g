...
def analyze_weekly(df):
    ...
    df_weekly = pd.DataFrame(weekly_cumulative)
    
    # Consistency score (like compose: % weeks better than previous week)
...
def analyze_monthly(df):
    ...
    df_monthly = pd.DataFrame(monthly_cumulative)
    
    # Consistency score (like compose: % months better than previous month)
...
def main():
    ...
    # Weekly analysis
    df_weekly, weekly_consistency = analyze_weekly(df)
    print_table(df_weekly, "WEEKLY PERFORMANCE")
    ...
    # Monthly analysis
    df_monthly, monthly_consistency = analyze_monthly(df)
    print_table(df_monthly, "MONTHLY PERFORMANCE")
