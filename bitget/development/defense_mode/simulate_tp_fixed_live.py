#!/usr/bin/env python3
"""
Compare MA5*1.02 Filter on 00 (raw) vs E1 (with LAYER 1)
Synthetic comparison: Before/After for each file
"""
...
def get_btc_regime(btc_df, trade_time):
    """Get BTC regime at trade time"""
    ...
    if pd.isna(last['ma5']):
        return None
    return last['close'] > last['ma5'] * 1.02
...
def evaluate_file(filepath, btc_df):
    """Evaluate single file with MA5*1.02 filter"""
    ...
    for _, trade in df.iterrows():
        ...
        is_bullish = get_btc_regime(btc_df, trade['open_time'])
        
        if is_bullish is None:
            filtered_profits.append(profit)
        elif (is_bullish and direction == 'LONG') or (not is_bullish and direction == 'SHORT'):
            filtered_profits.append(profit)
...
def main():
    print("="*120)
    print("MA5*1.02 FILTER COMPARISON: 00 (raw) vs E1 (with LAYER 1)")
    print("="*120)
    ...
    if total_00_change > 0:
        print(f"\n00 (raw):        ✅ MA5*1.02 improves by ${total_00_change:.2f}")
    else:
        print(f"\n00 (raw):        ❌ MA5*1.02 worsens by ${total_00_change:.2f}")
    
    if total_e1_change > 0:
        print(f"E1 (+ LAYER 1):  ✅ MA5*1.02 adds value: +${total_e1_change:.2f}")
        print(f"\n💡 RECOMMENDATION: Use BOTH layers (LAYER 1 + LAYER 2)")
    else:
        print(f"E1 (+ LAYER 1):  ❌ MA5*1.02 does NOT add value: ${total_e1_change:.2f}")
        print(f"\n💡 RECOMMENDATION: Use LAYER 1 only (skip LAYER 2)")
