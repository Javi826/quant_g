...
def evaluate_rule(df_trades, btc_df, ma_type, threshold, verbose=False):
    ...
    if verbose:
        print("\n" + "="*140)
        print(f"TRADE-BY-TRADE ANALYSIS: {ma_type.upper()} * {threshold:.2f}")
        print("="*140)
        print(f"{'#':<5} {'Trade Time':<20} {'BTC 1D Time':<20} {'Symbol':<12} {'Dir':<6} {'Profit':<10} "
              f"{'BTC 1D Close':<15} {f'{ma_type.upper()}':<12} {'Threshold':<12} {'Decision':<10}")
        print("-"*140)
    ...
    for idx, trade in df_trades.iterrows():
        ...
        allowed = should_allow_trade(direction, btc_regime, ma_type, threshold)
        
        if verbose and btc_regime is not None:
            btc_1d_time = btc_regime['ts']
            btc_close = btc_regime['close']
            ma_value = btc_regime[ma_type]
            threshold_value = ma_value * threshold
            decision = "✅ ALLOW" if allowed else "❌ BLOCK"
            
            print(f"{idx+1:<5} {str(trade_time):<20} {str(btc_1d_time):<20} {symbol:<12} {direction:<6} ${profit:<9.2f} "
                  f"${btc_close:<14,.2f} ${ma_value:<11,.2f} ${threshold_value:<11,.2f} {decision:<10}")
...
def main():
    ...
    print("\n" + "="*130)
    try:
        show_verbose = input("\n🔍 Show trade-by-trade details for best rule? (y/n): ").strip().lower()
        if show_verbose == 'y':
            evaluate_rule(df_lab, btc_df, best_rule['ma_type'], best_rule['threshold'], verbose=True)
    except KeyboardInterrupt:
        print("\n\nSkipped verbose output")
